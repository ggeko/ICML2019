...
class ResNetLayers(link.Chain):
    ...
    def __init__(self, out_dim, pretrained_model, n_layers):
        ...
        if pretrained_model and pretrained_model.endswith('.caffemodel'):
            _retrieve(out_dim, n_layers, 'ResNet-{}-model.npz'.format(n_layers),
    ...
    def convert_caffemodel_to_npz(cls, path_caffemodel, path_npz,
        ...
        chainermodel = cls(out_dim=out_dim,
                           pretrained_model=None, n_layers=n_layers)
        if n_layers == 50:
    ...
    def get_act(self, x):
        ...
        h, act = self.res5.get_act(h, act)
        
        self.output = ((self.fc6(pool5(h))).data)[:,:self.out_dim]
    ...
    def cutoff(self, x, cut):
        ...
        h, cid = self.res5.cutoff(h, cut, cid)
        
        return self.fc6(pool5(h))[:,:self.out_dim]
...
class ResNet50Layers(ResNetLayers):
    ...
    def __init__(self, out_dim=10, pretrained_model='auto'):
        if pretrained_model == 'auto':
            pretrained_model = 'ResNet-50-model.caffemodel'
        super(ResNet50Layers, self).__init__(out_dim, pretrained_model, 50)
...
class ResNet101Layers(ResNetLayers):
    ...
    def __init__(self, out_dim=10, pretrained_model='auto'):
        ...
        super(ResNet101Layers, self).__init__(out_dim, pretrained_model, 101)
...
class ResNet152Layers(ResNetLayers):
    ...
    def __init__(self, out_dim=10, pretrained_model='auto'):
        ...
        super(ResNet152Layers, self).__init__(out_dim, pretrained_model, 152)
...
def _make_npz(path_npz, path_caffemodel, model, out_dim, n_layers):
    sys.stderr.write(
    ...
    ResNetLayers.convert_caffemodel_to_npz(path_caffemodel, path_npz,
                                           out_dim, n_layers)
    npz.load_npz(path_npz, model)
    return model


def _retrieve(out_dim, n_layers, name_npz, name_caffemodel, model):
