...
############################################################################
out_dir = 'VGG_SGD_One_Manual_Fluc_Margin'
...
# for VGG networks
############################################################################
model_list = ['VGG_C3L3_WOB', 'VGG_C4L3_WOB', 'VGG_C5L3_WOB',
              'VGG_C6L3_WOB', 'VGG_WOB']
############################################################################
wide_factor = 1
############################################################################
epoch_list = [2500, 2500, 2500, 2500, 2500]
batch_list = [ 500,  500,  500,  500,  500]
trig_list  = [  20,   20,   20,   20,   20]
############################################################################
...
############################################################################
model_list = ['VGG_C3L3_WOB', 'VGG_C5L3_WOB', 'VGG_WOB']
epoch_list = [2500, 2500, 2500]
batch_list = [1000, 1000, 1000]
trig_list  = [  20,   20,   20]
############################################################################
...
def get_model(model_name):
    if model_name == 'VGG_WOB':
        model = VGG_WOB(wide_factor, num_classes)
    elif model_name == 'VGG_C6L3_WOB':
        model = VGG_C6L3_WOB(wide_factor, num_classes)
    elif model_name == 'VGG_C5L3_WOB':
        model = VGG_C5L3_WOB(wide_factor, num_classes)
    elif model_name == 'VGG_C4L3_WOB':
        model = VGG_C4L3_WOB(wide_factor, num_classes)
    elif model_name == 'VGG_C3L3_WOB':
        model = VGG_C3L3_WOB(wide_factor, num_classes)
    # end if
    return model
############################################################################
...
def train_model(train_bat, lr, epoch, model, data_ds, x_train, t_train,
                ...
                ep_list, model_name, batch, l2_init,
                log_name, loss_name, acc_name):
    ########################################################################
    setting = 'Model:{}, '.format(model_name)
    ...
    for eid in range(min([ep_init, epoch])):
        ...
        if ((eid % num_trig) == 0):
            ################################################################
            print('\nModel:{}, '.format(model_name), end='', flush=True)
    ...
    # end for
    ########################################################################
    save_name = '_{}.npy'.format(model_name)
...
def plot_epoch(eid, train_data, test_data, out_dir, file_name, data_type):
    ...
    ax.plot(e_list, train_data, label='train')
    ax.plot(e_list, test_data, label='test')
    ax.set_xlabel('epoch')
...
def plot_diff(ep_list, g_list, h_list, h_err,
    ...
    # end if
    ########################################################################
    model_desc = '{}, '.format(model_name) + co_dt
    ########################################################################
    filename = '{}/{}_{}sgd_diff.png'.format(out_dir, model_name, su_dt)
    ########################################################################
    plt.cla()
    fig = plt.figure()
    ax = fig.add_subplot(111)
...
############################################################################
def plot_err(ep_list, d_list, err_list, out_dir, data_name, model_name):
    ########################################################################
    if (data_name != ''):
        su_dt = data_name + '_'
        co_dt = data_name + ' '
    ...
    model_desc = '{}, '.format(model_name) + co_dt
    ########################################################################
    filename = '{}/{}_{}sgd_err.png'.format(out_dir, model_name, su_dt)
    ########################################################################
    plt.cla()
    fig = plt.figure()
    ax = fig.add_subplot(111)
...
############################################################################
def plot_data(ep_list, d_list, out_dir, data_name, model_name):
    ########################################################################
    if (data_name != ''):
        su_dt = data_name + '_'
        co_dt = data_name + ' '
    ...
    model_desc = '{}, '.format(model_name) + co_dt
    ########################################################################
    filename = '{}/{}_{}sgd_single.png'.format(out_dir, model_name, su_dt)
    ########################################################################
    plt.cla()
    fig = plt.figure()
    ax = fig.add_subplot(111)
...
def plot_mrg(marg_mean, marg_std, fluc_mean, fluc_std,
             ep_list, out_dir, model_name):
    #########################################################################
    model_desc = 'Margin vs Fluctuation: {}'.format(model_name)
    #########################################################################
    filename = '{}/margin_fluc_{}.png'.format(out_dir, model_name)
    plt.cla()
    fig = plt.figure()
    ax = fig.add_subplot(111)
    #########################################################################
...
def main():
    ...
    train_ds, test_ds = get_dataset(
        ds_name, 'VGG', data_dir, ds_size, ds_normalized)
    ########################################################################
    ...
    ## setup l2_init
    ########################################################################
    model_name = 'VGG_C3L3_WOB'
    print('setup initial l2 values by the first network =', model_name)
    ########################################################################
    ...
    ########################################################################
    for model_name, epoch, trig, batch in zip(
            model_list, epoch_list, trig_list, batch_list):
        ####################################################################
        num_trig = trig
        ####################################################################
        ...
        ep_list = []
        ####################################################################
        log_name  = 'log_{}.txt'.format(model_name)
        loss_name = 'loss_{}.png'.format(model_name)
        acc_name  = 'accuracy_{}.png'.format(model_name)
        ####################################################################
        train_model(train_bat, lr, epoch, model, data_ds, x_train, t_train,
                    ...
                    log_name, loss_name, acc_name)
        ####################################################################
        data_dict[model_name] = make_data_dict()
        data_dict[model_name]['ep_list'] = ep_list
        ####################################################################
        data_dict[model_name]['switch_mean'].append(b_data['switch_mean'])
        data_dict[model_name]['switch_std'].append(b_data['switch_std'])
        ####################################################################
        data_dict[model_name]['g_sigma_std'].append(b_data['g_sigma_std'])
        ####################################################################
        data_dict[model_name]['g_fluc_std'].append(b_data['g_fluc_std'])
        ####################################################################
        data_dict[model_name]['h_sigma_mean'].append(b_data['h_sigma_mean'])
        data_dict[model_name]['h_sigma_std'].append(b_data['h_sigma_std'])
        ####################################################################
        data_dict[model_name]['h_fluc_mean'].append(b_data['h_fluc_mean'])
        data_dict[model_name]['h_fluc_std'].append(b_data['h_fluc_std'])
        ####################################################################
        plot_err(ep_list, b_data['switch_mean'], b_data['switch_std'],
                 out_dir, 'switch', model_name)
        ####################################################################
        plot_err(ep_list, b_data['h_sigma_mean'], b_data['h_sigma_std'],
                 out_dir, 'h_sigma', model_name)
        plot_err(ep_list, b_data['h_fluc_mean'], b_data['h_fluc_std'],
                 out_dir, 'h_fluc', model_name)
        ####################################################################
    ...
    # end for
    ########################################################################
    with open('{}/vgg_sgd_one_manual_fluc_margin.pkl'.format(
            out_dir), 'wb') as f:
